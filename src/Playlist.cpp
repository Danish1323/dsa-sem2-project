...
Song *Playlist::getHead() const { return head; }

Song *Playlist::getTail() const { return tail; }

Song *Playlist::getCurrent() const { return current; }

int Playlist::getSongCount() const { return songCount; }

void Playlist::setCurrent(Song *song) { current = song; }
...
void Playlist::insertAtBeginning(std::string title, std::string artist,
                                 std::string album) {
...
void Playlist::insertAtEnd(std::string title, std::string artist,
                           std::string album) {
  Song *newSong = new Song(title, artist, album);
  if (!tail) {
    head = tail = current = newSong;
...
bool Playlist::insertAfter(const std::string &afterTitle, std::string title,
